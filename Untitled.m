...
data(:,3) = data(:,3)/100;
data_put = csvread('put.csv',1,0); %date strike p left (s shibor)
data_call = csvread('call.csv',1,0); %date strike c left (s shibor)
data_put(data_put(:,4)<=1,:) = [];          %消除调仓效应
...
for i = 1:length(data_call)
    ...
    T = data_call(i,4);
    c = data_call(i,3);
    %d1 = (log(S/K)+r*T)/(x*T^0.5) + 0.5*x*T^0.5;
    %d2 = (log(S/K)+r*T)/(x*T^0.5) + 0.5*x*T^0.5 - x*T^0.5;
    %yy = inline('S*normcdf((log(S/K)+r*T)/(x*T^0.5) + 0.5*x*T^0.5) - K*exp(-r*T)*normcdf((log(S/K)+r*T)/(x*T^0.5) + 0.5*x*T^0.5 - x*T^0.5) - c');
